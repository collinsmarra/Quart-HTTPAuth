...
class HTTPAuth(object):
    ...
    def error_handler(self, f):
        ...
        async def decorated(*args, **kwargs):
            res = await self.ensure_async(f)(*args, **kwargs)
            ...
            if check_status_code and res.status_code == 200:
                # if the user didn't set the status code, use 401
                res.status_code = 401
            if "WWW-Authenticate" not in res.headers.keys():
                res.headers["WWW-Authenticate"] = self.authenticate_header()
            return res

        self.auth_error_callback = decorated  # Assign the decorated function
        return decorated
    ...
    def get_auth(self):
        ...
        if self.header is None or self.header == "Authorization":
            ...
            if (
                auth is None and "Authorization" in request.headers
            ):  # pragma: no cover
    ...
    async def get_auth_password(self, auth):
        password = None
    ...
    async def authorize(self, role, user, auth):
        if role is None:
            return True
...
class HTTPDigestAuth(HTTPAuth):
    ...
    def authenticate_header(self):
        ...
        if self.qop:
            return (
                '{0} realm="{1}",nonce="{2}",opaque="{3}",algorithm="{4}"'
                ',qop="{5}"'
            ).format(
                self.scheme,
                self.realm,
    ...
    async def authenticate(self, auth, stored_password_or_ha1):
        if (
            not auth
        ...
        if self.use_ha1_pw:
            ...
            a1 = (
                auth.username + ":" + auth.realm + ":" + stored_password_or_ha1
            )
                ...
                if not self.main_auth.is_compatible_auth(request.headers):
                    for auth in self.additional_auth:
                        if auth.is_compatible_auth(request.headers):
                            selected_auth = auth
                            break
                return await selected_auth.login_required(
